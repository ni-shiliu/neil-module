...
import com.neil.pay.utils.CertSerializeUtil;
...
public class CertificatesManager {
    ...
    public synchronized void putMerchant(String merchantId, Credentials credentials, byte[] apiV3Key)
            throws IOException, GeneralSecurityException, PayException {
    ...
    private synchronized void downloadAndUpdateCert(String merchantId, Verifier verifier, Credentials credentials,
                                                    byte[] apiV3Key) throws IOException, GeneralSecurityException, PayException {
        try (CloseableHttpClient httpClient = WechatPayHttpClientBuilder.create()
                .withCredentials(credentials)
                .withValidator(verifier == null ? (response) -> true
                        : new WechatPayValidator(verifier))
                .withProxy(proxy)
                .build()) {
            ...
            try (CloseableHttpResponse response = httpClient.execute(httpGet)) {
                ...
                if (statusCode == SC_OK) {
                    ...
                    throw new PayException("下载平台证书返回状态码异常，状态码为:" + statusCode);
                }
            }
        }
    }
